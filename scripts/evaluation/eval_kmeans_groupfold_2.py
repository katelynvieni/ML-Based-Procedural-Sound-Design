...
OUT_DIR = REPO_ROOT / "evaluation" / "Kmeans_groupfold"
...
def parse_segment_pairs(seg_data):
    ...
    if (
        isinstance(seg_data, list)
        and len(seg_data) == 2
        and all(isinstance(x, (int, float)) for x in seg_data)
    ):
        s, e = seg_data
...
def rebuild_labeled_windows_X_meta():
    ...
    meta["key"] = (
        meta["rel"].astype(str)
        + "||"
        + meta["window_start"].astype(str)
        + "||"
        + meta["window_end"].astype(str)
    )
    return X, meta
...
def plot_one_scatter_pca(df, out_path):
    ...
    marker_handles = [
        Line2D([], [], marker=marker_map[p], linestyle="", color="black", label=f"pred={p}")
        for p in pred_uniq
    ]
    leg1 = ax.legend(
    ...
    color_handles = [
        Line2D([], [], marker="o", linestyle="", color=color_map[t], label=f"true={t}")
